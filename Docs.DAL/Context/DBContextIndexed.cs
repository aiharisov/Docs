...
namespace Docs.DAL.Context
{
    public abstract class DbContextIndexed : DbContext
    {
        ...
        private void BuildingFullTextIndexes(string tableName, IEnumerable<PropertyInfo> propertyes)
        {
            ...
            string Id = "Id";
            if (!string.IsNullOrEmpty(fullTextColumns))
            {
                //Создаем полнотекстовый индекс
                Database.ExecuteSqlCommand(System.Data.Entity.TransactionalBehavior.DoNotEnsureTransaction,
                    $@"IF NOT EXISTS (SELECT * FROM sysindexes WHERE id=object_id('{tableName}') and name='IX_{Id}') CREATE UNIQUE INDEX IX_{Id} ON {tableName} ({tableName}{Id});CREATE FULLTEXT CATALOG FTXC_{tableName} AS DEFAULT;CREATE FULLTEXT INDEX ON {tableName}({fullTextColumns}) KEY INDEX [IX_{Id}] ON ([FTXC_{tableName}]) WITH STOPLIST = SYSTEM;");
            }
        }
